...
allids = {l: set(re.findall(r'id="([^"]+)"', open(os.path.join(ROOT, f"book-{l}.html")).read())) for l in "irsdpt"}
for l in "irsdpt":
    t = open(os.path.join(ROOT, f"book-{l}.html")).read()
    for target, anchor in re.findall(r'href="book-([irsdpt])\.html#([^"]+)"', t):
